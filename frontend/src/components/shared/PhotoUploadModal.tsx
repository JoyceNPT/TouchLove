...
const PhotoUploadModal: React.FC<PhotoUploadModalProps> = ({ isOpen, onClose, coupleId, onSuccess }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(`/memories/${coupleId}`, formData, {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Có lỗi xảy ra khi tải lên');
      }
    } catch (err: any) {
          ...
          >
            ...
            <div className="p-6 flex items-center justify-between border-b border-border">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Tối đa {MAX_FILES} ảnh/video · Mỗi tệp tối đa {MAX_SIZE_MB}MB</p>
              </div>
              ...
              >
                {activeFile ? (
                  ...
                  <>
                    ...
                    <p className="font-medium text-sm">Nhấn để chọn ảnh hoặc video</p>
                    <p className="text-xs text-muted-foreground mt-1">Có thể chọn nhiều tệp cùng lúc</p>
                  </>
                )}
              </div>
